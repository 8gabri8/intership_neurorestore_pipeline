...
*) colobars = we do not advive to use colorbars as they will refer only to one hemisphere (nevwetheles yuo can choose to diplaye them in the dunction my_plot())
    Attention: in case you use or not the colorbarp, you have to chnage the ratios in the function split_and_merge_png_images() to merge the 2 images in the correct center
...
for i, csv_file in enumerate(csv_files):
    ...
    if side != "Merge":
        ...
        for cut in range(start_cut, end_cut, step):
            ...
            print(f"Porcessing cut number {cut}")
            ...
            fig = f.my_plot(show_colorbar=False, show_legend=False,xlabel="", ylabel="", hide_axes=True) 
            ...
            fig_path = os.path.join(heatmaps_dir, side, f'{cut}.png')
            fig.savefig(fig_path, dpi=100)
            print(f"\nSaved figure: {cut}.png\n")
        ...
        for root, dirs, files in os.walk(heatmaps_dir_left):
            for file in files:
                ...
                split_and_merge_png_images(image_path_left=file_path_right, #ATTENTION, are inverted
                                           ...
                                           left_ratio=0.513,right_ratio=0.487)
                
                #ratios
                    # if colormap is there: left_ratio=0.493,right_ratio=0.507
                    # if colormap is NOT there: left_ratio=0.5,right_ratio=0.5
