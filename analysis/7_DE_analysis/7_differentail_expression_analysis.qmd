...
- The significance of a region is determined using the adjusted p-value (p-value-adj).\
...
data_summary <- function(data, varname, groupnames){
  ...
  summary_func <- function(x, col){
    ...
    c(mean = mean_val, std = sd_val, se = se_val)
  }
...
for (arch in archetypes){
  ...
  # calculate mean, std and sem for each ROI
  ...
  for ( roi in ROI_significant_arch ){
    ...
    plot <- ggplot(df_roi, aes(x = TimePoint, y = mean, group = ROI)) +
      ...
      geom_errorbar(aes(ymin = mean - se, ymax = mean + se), width = 0.2, color = "red") +
      geom_point(color = "blue") +
      labs(x = "Timepoint", y = "Mean Synapses Count", 
           title = paste0("Evolution of ROI synapses count\n", roi, "\nInjection Region: ", region_injection))

    print(plot)
    
    #Save the figure
    roi_clean <- gsub("[, :/]", "", roi)
    path_plot = paste0(save_path, "/Lineplots/", arch, "/", roi_clean, ".pdf")
    ggsave(path_plot, plot, width = 5, height = 5)
